...
AP3216C 软件包提供了使用近距离感应（ps）与光照强度（als）传感器 `ap3216c` 基本功能，并且提供了硬件中断的可选功能。本文介绍该软件包的基本功能，以及 `Finsh/MSH` 测试命令等。

基本功能主要由传感器 `ap3216c` 决定：提供可以自由配置的四种不同量程光照强度检测功能与四种不同检测增益近距离感应功能，并且提供相应的校准功能；另外，能够选择使用光照强度检测与近距离感应的硬件中断，并且分别可以设置最大、最小阈值的中断条件。
...
通过 `ap3216c` 传感器读取近距离感应测量值，返回 `16` 位近距离感应值，具体参数与返回说明如下表
...
ap3216c 软件包提供了丰富的测试命令，项目只要在 RT-Thread 上开启 Finsh/MSH 功能即可。在做一些基于 `ap3216c` 的应用开发、调试时，这些命令会非常实用，它可以准确的读取指传感器测量的光照强度与近距离感应。具体功能可以输入 `ap3216c` ，可以查看完整的命令列表
...
探测成功之后，输入 `ap3216c read` 即可获取光照强度与近距离感应检测值，包括提示信息，日志如下： 

```
msh />ap3216c read            #近距离没有探测到物体
object is not proximity of sensor
ap3216c measure current brightness: 25.9 (lux)  
msh /> 
msh />ap3216c read            #近距离有物体
